...
stable_lqr = {'Yes','Yes','Yes','Yes','Yes','Yes','Yes','No','Yes','Yes',};
stable_dl = {'Yes','No','Yes','Yes','Yes','Yes','Yes','Yes','Yes','No',};
...
method = 2;
file_to_read = 'DL_+7_roll_test' + string(test) + '.csv';
if isequal('LQR',meth)
    method = 1;
    file_to_read = 'LQR_+7_roll_test'+ string(test) + '.csv';
end
...
%     if abs(pitch_data(i)) < ptch_limit && abs(roll_data(i)) < roll_limit && abs(pitchvel_data(i)) < rollvel_limit && abs(rollvel_data(i)) < rollvel_limit
    if abs(roll_data(i)) < roll_limit && abs(disk_data(i)) < disk_limit
...
% end
plot([time_stable time_stable],[-10,10],'black--')
...
plot(t,disk_data,'b-')
plot(t,wheel_data,'r-')
plot([time_stable time_stable],[-400,400],'black-','HandleVisibility','off')
...
plot([time_stable time_stable],[-12,12],'black-','HandleVisibility','off')
...
fprintf("%d\t&%s\t&%.4f\t&%.4f\t&%.4f\t&%.4f\t\\\\ \\hline \n " ,...
    test,stabl{method}{test},init_roll,init_rollvel,init_pitch,init_pitchvel)

fprintf("%s\t&%d\t&%.4f\t&%.4f\t&%.4f\t&%.4f\t\\\\ \\hline \n " ,...
    meth ,test,time_stable,max_oversht,init_roll,init_rollvel)
