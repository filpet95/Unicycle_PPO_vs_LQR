...
deffilem5sub
% addpath('model_tests')
file_to_read = 'LQR_+7_roll_test1.csv';
...
for i = 1:10
    file_to_read = 'LQR_+7_roll_test' + string(i) + '.csv';
    tst_data = csvread(file_to_read,1,0);
    enable_data = tst_data(2:end,10);
    enable_idx = find(enable_data,1);
    pitch_data = tst_data(2:end,9);
...
for i = 1:10
    file_to_read = 'DL_+7_roll_test' + string(i) + '.csv';
    tst_data = csvread(file_to_read,1,0);
    enable_data = tst_data(2:end,10);
    enable_idx = find(enable_data,1);
    pitch_data = tst_data(2:end,9);
...
table(abs(init_rad_LQR*180/pi - [7*ones(10,1),zeros(10,1)])<=0.3,abs(init_rad_DL*180/pi - [7*ones(10,1),zeros(10,1)])<=0.3,...
