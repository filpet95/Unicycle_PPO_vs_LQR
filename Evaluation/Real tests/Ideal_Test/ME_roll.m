...
MEAN_V_LQR = 1.4972;
MEAN_V_DL = 1.2008;
stable_lqr = {'Yes','Yes','Yes','No','Yes','Yes','Yes','Yes','Yes','Yes'};
stable_dl = {'Yes','Yes','Yes','Yes','Yes','Yes','No','No','Yes','No'};
...
method = 2;
file_to_read = 'DL_modelerror_9_test' + string(test) + '.csv';
if isequal('LQR',meth)
    method = 1;
    file_to_read = 'LQR_modelerror_9_test'+ string(test) + '.csv';
end
...
mean_roll = "-";
if isequal('Yes',stabl{method}{test})
    roll_offset_data = roll_data(200:end);
    mean_roll = mean(roll_offset_data);
    mean_roll = string(mean_roll);
end
...
fprintf("%d\t&%s\t&%s\t&%.4f\t&%.4f\t&%.4f\t&%.4f\t\\\\ \\hline \n " ,...
    test,stabl{method}{test},mean_roll,init_roll,init_rollvel,init_pitch,init_pitchvel)
% end
% roll_limit = 2;
% ptch_limit = 2;
% ptchvel_limit = 15;
