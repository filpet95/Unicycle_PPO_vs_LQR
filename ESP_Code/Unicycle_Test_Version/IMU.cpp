...
void IMU::update_state(float Ts, double *roll_ptr, double *pitch_ptr, double *roll_vel_ptr, double *pitch_vel_ptr)
{
  ...
  roll_acc = -acc2roll(imu.calcAccel(ax), imu.calcAccel(ay), imu.calcAccel(az)) - 0.01547;//- 0.0079;//-0.01047;//-0.00873;// - 0.0271;//-0.0175;
  pitch_acc = -acc2pitch(imu.calcAccel(ax), imu.calcAccel(az)) + 0.014;//0.0267;//+ 0.014;// 0.0297;// + 0.0175;//75
  ...
  /*//Pitch is defined in the rotated roll frame, i.e. same frame as the IMU is in.
    Serial.print(roll_meas*180/PI);
    Serial.print(" ");
    Serial.print(pitch_meas*180/PI);
    Serial.print(" ");
  */
  ...
  /*roll_w = roll_lambda*roll_w + 1;
    roll_vel = (1-1/roll_w)*roll_vel + (1/roll_w)*roll_meas;
    pitch_w = pitch_lambda*pitch_w + 1;
    pitch_vel = (1-1/pitch_w)*pitch_vel + (1/pitch_w)*pitch_meas;*/
...
void IMU::kalman_update(double* omega_hat, double* domega_hat, double P[2][2], double gyro_meas) {
  //*domega_hat = *domega_hat - ((P[0][0]*gyro_meas+(*omega_hat)*R_VAL)/(P[0][0]+R_VAL));
