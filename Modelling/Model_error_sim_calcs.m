...
p_b0 = simplify(A_0w*[0;Lwb_y;Lwb;1]);
...
[I,m] = get_parameters_model_error();
Mw = m.m_wheel;
Mb = m.m_body;
Md = m.m_disk;
%velocity vector
...
f = matlabFunction(f,'File','NL_model_model_error');
...
%% Controller Design
%stablized 2secs, first movie. 
% Q = diag([1 eps 200 1 1e2 14.5903]);
% R = 12*diag([1e-3 1e-3]);
% Q = diag([1e-3 1e-3 200 1 1e2 14.5903]);
% R = 12*diag([1e-3 1e-2]);
Q = diag([1e-3 1e-3 200 1 1e2 10]);
% Q = diag([eps 1e-3 200 100 1e2 eps]);
R = 12*diag([1e-3 1e-2]);
...
% sig_imu = 1e-6;
% sig_imu = 0.002;
% Q_k = diag([sig_inp,sig_inp]);
% R_k = diag([sig_alpha,sig_alpha,sig_imu,sig_imu]);
% R_k = diag([sig_alpha,sig_alpha,1e5,1e5,sig_imu,sig_imu,1e5,1e5]);
% Using sig_imu = sig_alpha -> weird IMU dep. on disk speed gone. However,
% slow angles!
% Using Ltest -> also smaller IMU dep.
